...
4. **If you learn something lasting:** write it to the relevant note following the rules in `standards/vault-notes.md`. Then run `python "$VAULT_ENGINE/tools/graph.py" lint` and commit in English. If the vault has a remote (`git remote` prints something), also run `git pull --rebase && git push`; otherwise the commit is enough. The guard hook blocks commits containing personal data or secrets.
